...
// ########################################
std::string READ_DATA_FOLDER = "/home/erik/Documents/jaar3-bach/bachelorthesis/code/data/dram_read_data/";
std::string WRITE_DATA_FOLDER = "/home/erik/Documents/jaar3-bach/bachelorthesis/code/data/dram_write_data/";
std::string READ_WRITE_INFO_FILE = "/home/erik/Documents/jaar3-bach/bachelorthesis/code/data/read_write_info.txt";
std::string READ_DATA_HEADER = "time,cores\n";
std::string WRITE_DATA_HEADER = "time,cores\n";
UInt32 read_write_newfile = 0;
// ########################################
...
// ##############
UInt64 read_access_count_per_core[MAX_NUM_OF_CORES];
UInt64 write_access_count_per_core[MAX_NUM_OF_CORES];
// ##############
...
#define ACCUMULATION_TIME     (10)    // Till 200 us bank counts will be accumalated
...
dram_read_trace(IntPtr address, core_id_t requester, SubsecondTime now, UInt64 m_reads)
{
    ...
    if(Sim()->getMagicServer()->inROI())
    {
        ...
        // ########################################
        ++read_access_count_per_core[requester];
        // ########################################
        ...
        if (current_time > ACCUMULATION_TIME + read_interval_start_time){
            ...
            // struct dirent *read;
            // DIR *dir;
            // dir = opendir(READ_WRITE_DATA_PATH + "read_data/".c_str());
            // if (dir != NULL) {
            ...
            // int filenumber = i;
            // if (read_interval_start_time == 0) {
            //     ++filenumber;
            // }

            std::fstream info_file(READ_WRITE_INFO_FILE, std::ios_base::in);
            UInt32 filenumber;
            info_file >> filenumber;
            if (read_write_newfile == 0) {
                ofstream f ((READ_WRITE_INFO_FILE).c_str());
                if (f.is_open()) {
                    f << ++filenumber;
                }
                ++read_write_newfile;
                ofstream fr ((READ_DATA_FOLDER + "dram_read_data" + std::to_string(filenumber) + ".csv").c_str(), ios::app);
                if (fr.is_open()) {
                    fr << READ_DATA_HEADER;
                }
                ofstream fw ((WRITE_DATA_FOLDER + "dram_write_data" + std::to_string(filenumber) + ".csv").c_str(), ios::app);
                if (fw.is_open()) {
                    fw << WRITE_DATA_HEADER;
                }
            }
            ofstream file ((READ_DATA_FOLDER + "dram_read_data" + std::to_string(filenumber) + ".csv").c_str(), ios::app);
            if (file.is_open()) {
                file << read_interval_start_time << ",";
                for(UInt32 i = 0; i < MAX_NUM_OF_CORES; i = i + 1){
                    file << read_access_count_per_core[i] << " " ;
                    read_access_count_per_core[i] = 0;
                }
                file << "\n";
                file.close();
...
dram_write_trace(IntPtr address, core_id_t requester, SubsecondTime now, UInt64 m_writes)
{
    ...
    if(Sim()->getMagicServer()->inROI())
    {
        ...
        // ########################################
        ++write_access_count_per_core[requester];
        // ########################################
        ...
        if (current_time > ACCUMULATION_TIME + write_interval_start_time){
            ...
            ofstream file ((WRITE_DATA_FOLDER + "dram_write_data" + std::to_string(filenumber) + ".csv").c_str(), ios::app);
            if (file.is_open()) {
                file << write_interval_start_time << ",";
                for(UInt32 i = 0; i < MAX_NUM_OF_CORES; i = i + 1){
                    file << write_access_count_per_core[i] << " " ;
                    write_access_count_per_core[i] = 0;
                }
                file << "\n";
                file.close();
            } else cout << "Unable to open file\n";
