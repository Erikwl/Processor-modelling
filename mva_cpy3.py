...
def mva(N, refs, visits, caps, service_times):
    ...
    nrs = np.zeros((nr_of_states, M))
    utils = np.zeros((nr_of_states, M))
    probs = np.zeros((nr_of_states, M, min(np.sum(N) + 1, max(caps))))
    waits = np.zeros((nr_of_states, M, R))
    throughputs = np.zeros((nr_of_states, R))
    ...
    for N_ in range(nr_of_states):
        ...
        for r, Nr in enumerate(N):
            ...
            for i in Ms[r]:
                ci = caps[i]
                if Nr_:
                    waits[N_, i, r] = 1 / (ci * mus[i]) * ( \
                        1 \
                        + nrs[r_cus_removal, i] \
                        + np.sum((ci - n - 1) * probs[r_cus_removal, i, n]
                                 for n in range(1 + min(ci - 2, totalN_ - 1))))

            if Nr_:
                throughputs[N_,r] = Nr_ / np.sum(visits[i,r] * waits[N_, i,r] for i in Ms[r])
                # print('throughput ', r, throughputs[r,N_])

        for i in range(M):
            nrs[N_,i] = np.sum(visits[i,r] * throughputs[N_,r] * waits[N_, i,r,] for r in Rs[i])
            utils[N_,i] = 1 / mus[i] * np.sum(throughputs[N_,r] * visits[i,r] for r in Rs[i])

            ci = caps[i]
            for n in range(1, 1 + min(ci - 1, totalN_)):
                # print(i, n, '\n')
                probs[N_,i,n] = 1 / (min(n, ci) * mus[i]) \
                    * np.sum(visits[i,r] * throughputs[N_,r] * probs[N_ - N_products[r],i,n - 1]
                             for r in Rs[i])
                # r = 1
            ...
            probs[N_,i,0] = 1 - 1 / ci * (
                utils[N_,i] + np.sum((ci - n) * probs[N_,i,n] for n in range(1,1+min(ci-1,totalN_)))
            )
